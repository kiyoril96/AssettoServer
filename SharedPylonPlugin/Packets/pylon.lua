...
function closeCallback(okClicked)
    local close = okClicked

    return close
end

ui.registerOnlineExtra(ui.Icons.Attention
    ,'pylon'
    ,availableCallback
    ,uiCallback
    ,closeCallback
...
local SharedPylonEvent = ac.OnlineEvent({
    ...
    if sender == nil then 
        addOne(message.position,rgbm(1,0,0,1) ,true) 
    end
end
)

function addOne(pos,TextureCollar ,flag)

    --if  ac.checkAdminPrivileges() then end

    if ac.getSim().isOnlineRace and SharedPylonEvent ~= nil and not flag then
        SharedPylonEvent{position=pos}
    end
    ...
    web.loadRemoteModel('https://github.com/kiyoril96/kiyo-eng_acApps/releases/download/anywherePyron_2_1/kiyo-eng_anywherePyron.zip', function(err,name)
